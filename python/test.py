...
masks=numpy.random.random_integers(0,1, size=(12,1,815,815)).astype(numpy.int32) 
...
net['conv2']=lasagne.layers.Conv2DLayer(net['data'], num_filters=1, filter_size=5)
...
prediction = lasagne.layers.get_output(net['conv2'])
loss = lasagne.objectives.squared_error(prediction, target_var)
loss = loss.mean()

#Also add weight decay to the cost function
weightsl2 = lasagne.regularization.regularize_network_params(net['conv2'], lasagne.regularization.l2)
loss += weight_decay * weightsl2

params = lasagne.layers.get_all_params(net['conv2'], trainable=True)
updates = lasagne.updates.sgd(loss, params, learning_rate=lr)

train_fn = theano.function([input_var, target_var], loss, updates=updates)

test_prediction = lasagne.layers.get_output(net['conv2'], deterministic=True)
